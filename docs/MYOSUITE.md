...
**envpool <= 1.2.5 resets are deterministic** — no state/target randomization, so the `*Random` variants train against a single fixed target and envpool reward overstates performance on the real task ([envpool#432](https://github.com/sail-sg/envpool/issues/432)). Fixed in envpool 1.2.6; policies trained on 1.2.5 must be retrained. Always evaluate final policies on native myosuite.
