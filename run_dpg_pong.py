"""Run 3 Pong experiments: baseline PPO, DPG-old, DPG-curr, then plot comparison."""
import subprocess
import sys
import os
import glob
...
def plot_comparison():
    ...
    for label, prefix, color in [
        ('PPO Baseline', 'Pong_baseline', 'blue'),
        ('DPG (old probs)', 'Pong_dpg_old', 'red'),
        ('DPG (curr probs)', 'Pong_dpg_curr', 'green'),
    ]:
        ...
        if not rtags:
            print(f"No reward tag for {label}, tags: {tags[:10]}")
            continue
    ...
    ax.set_title('Pong: PPO vs Delightful Policy Gradient (seed=42)')
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig('dpg_pong_comparison.png', dpi=150)
    print(f"\nPlot saved to dpg_pong_comparison.png")


if __name__ == "__main__":
    configs = [
        ("rl_games/configs/atari/ppo_pong_baseline.yaml", "Pong Baseline"),
        ("rl_games/configs/atari/ppo_pong_dpg_old.yaml", "Pong DPG (old)"),
        ("rl_games/configs/atari/ppo_pong_dpg_curr.yaml", "Pong DPG (curr)"),
    ]
