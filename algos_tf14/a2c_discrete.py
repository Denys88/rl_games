from common import tr_helpers, vecenv
...
class A2CAgent:
    def __init__(self, sess, base_name, observation_space, action_space, config, logger):
        ...
        if self.is_polynom_decay_lr:
            self.lr_multiplier = tf.train.polynomial_decay(1.0, self.epoch_num, config['max_epochs'],
                                                           end_learning_rate=0.001,
                                                           power=tr_helpers.get_or_default(config, 'decay_power', 1.0))
        if self.is_exp_decay_lr:
            self.lr_multiplier = tf.train.exponential_decay(1.0, self.epoch_num, config['max_epochs'],
        ...
        self.variables = TensorFlowVariables([self.target_action, self.target_state_values, self.target_neglogp],
                                             self.sess)
    ...
    def setup_losses(self):
        ...
        if (self.ppo):
            ...
            self.pg_loss_clipped = -tf.multiply(self.advantages_ph,
                                                tf.clip_by_value(self.prob_ratio, 1. - curr_e_clip, 1. + curr_e_clip))
        ...
        if self.clip_value:
            self.cliped_values = self.old_values_ph + tf.clip_by_value(
                tf.squeeze(self.state_values) - self.old_values_ph, - curr_e_clip, curr_e_clip)
        ...
        if self.is_adaptive_lr:
            self.current_lr = tf.where(self.kl_approx > (2.0 * self.lr_threshold),
                                       tf.maximum(self.current_lr / 1.5, 1e-6), self.current_lr)
            self.current_lr = tf.where(self.kl_approx < (0.5 * self.lr_threshold),
    ...
    def get_action_values(self, obs):
        ...
        if self.network.is_rnn():
            ...
            return self.sess.run(run_ops, {self.target_obs_ph: obs, self.target_states_ph: self.states,
    ...
    def get_masked_action_values(self, obs, action_masks):
        ...
        if self.network.is_rnn():
            run_ops.append(self.target_lstm_state)
            return self.sess.run(run_ops, {self.action_mask_ph: action_masks, self.target_obs_ph: obs,
                                           self.target_states_ph: self.states, self.target_masks_ph: self.dones})
        else:
    ...
    def get_values(self, obs):
        if self.network.is_rnn():
            return self.sess.run([self.target_state_values],
                                 {self.target_obs_ph: obs, self.target_states_ph: self.states,
                                  self.target_masks_ph: self.dones})
        else:
    ...
    def play_steps(self):
        ...
        for _ in range(self.steps_num):
            ...
            for reward, length, done, info in zip(self.current_rewards[::self.num_agents],
        ...
        if self.network.is_rnn():
            result = (
            *map(swap_and_flatten01, (mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs, mb_states)),
            epinfos)
        else:
            result = (
            *map(swap_and_flatten01, (mb_obs, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs)), None,
            epinfos)
        return result
            ...
            if self.network.is_rnn():
                ...
                for _ in range(0, mini_epochs_num):
                    ...
                    for i in range(0, num_minibatches):
                        ...
                        run_ops = [self.actor_loss, self.critic_loss, self.entropy, self.kl_approx, self.current_lr,
                ...
                for _ in range(0, mini_epochs_num):
                    ...
                    for i in range(0, num_minibatches):
                        ...
                        dict = {self.obs_ph: obses[batch], self.actions_ph: actions[batch],
                ...
                if len(self.game_rewards) > 0:
                    ...
                    if mean_rewards > last_mean_rewards:
                        ...
                        if last_mean_rewards > self.config['score_to_win']:
                            print('Network won!')
                            self.save(
                                "./nn/" + self.config['name'] + 'ep=' + str(epoch_num) + 'rew=' + str(mean_rewards))
                            return last_mean_rewards, epoch_num

                if epoch_num > max_epochs:
                    self.save(
                        "./nn/" + 'last_' + self.config['name'] + 'ep=' + str(epoch_num) + 'rew=' + str(mean_rewards))
