...
class MultiWalker(gym.Env):
    def __init__(self, name="multiwalker", **kwargs):
        ...
        self.action_space = self.env.action_space('walker_0')
        self.steps_count = 0
        obs_len = self.env.observation_space('walker_0').shape[0]
    ...
    def step(self, action):
        ...
        terms = np.stack([terminated['walker_0'], terminated['walker_1'], terminated['walker_2']])
        truncs = np.stack([truncated['walker_0'], truncated['walker_1'], truncated['walker_2']])
        if self.apply_agent_ids:
            num_agents = 3
        ...
        out_info = {}
        out_info['time_outs'] = truncs
        return obses, rewards, terms, truncs, out_info
