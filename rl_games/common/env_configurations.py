...
def create_goal_gym_env(**kwargs):
    ...
    env = gym.make(name, **kwargs)
    env = FlattenObservation(FilterObservation(
...
def create_atari_gym_env(**kwargs):
    ...
    env = wrappers.make_atari_deepmind(
        name, skip=skip, episode_life=episode_life, wrap_impala=wrap_impala, **kwargs)
    return env
...
def create_smac(name, **kwargs):
    ...
    if frames > 1:
        if has_cv:
            env = wrappers.BatchedFrameStackWithStates(
                env, frames, transpose=False, flatten=flatten)
        else:
            env = wrappers.BatchedFrameStack(
                env, frames, transpose=False, flatten=flatten)
...
def create_smac_cnn(name, **kwargs):
    ...
    if has_cv:
        env = wrappers.BatchedFrameStackWithStates(
            env, frames, transpose=transpose)
    else:
...
def create_minigrid_env(name, **kwargs):
    ...
    if rgb_fully_obs:
        ...
        env = gym_minigrid.wrappers.RGBImgPartialObsWrapper(
            env, tile_size=84//view_size)  # Get pixel observations
