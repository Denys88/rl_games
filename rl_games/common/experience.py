...
class VectorizedReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        idxs = torch.randint(0, self.capacity if self.full else self.idx, size=(batch_size,), device=self.device)

        obses = self.obses[idxs]#.detach().clone()
        actions = self.actions[idxs]#.detach()#.clone()
        rewards = self.rewards[idxs]#.detach()#.clone()
        next_obses = self.next_obses[idxs]#.detach()#.clone()
        not_dones = self.dones[idxs]#.detach()#.clone()
        not_dones_no_max = self.dones_no_max[idxs]#.detach()#.clone()
